...
module.exports = app =>{
    ...
    const remove = async (req,res) => {
        try{
            ...
            existsOrError(rowsDeleted,"Artigo nao encontrado");
            res.status(204).send();
        }catch(msg){
            res.status(500).send(msg);
        }
    }
    const limit = 10; //usado para limite de paginacao.
    ...
    const getByCategory = async (req,res) => {
        ...
        .limit(limit).offset(page * limit - limit).whereRaw('?? = ??',['u.id','a.userId'])
